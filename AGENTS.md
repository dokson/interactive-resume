...
Contact and identity data (email, phone, LinkedIn, GitHub, X/Twitter, Wikidata, employer, consulting brand, education, site URL/domain, profile image path) live **only** in `package.json`'s `config.contact` / `config.social` / `config.employer` / `config.consulting` / `config.education` / `config.images` blocks. Source files (`about.html`, `index.html`, `404.html`, `src/json/seo-meta.json`, `llms.txt`, `robots.txt`) reference these via `__PLACEHOLDER__` tokens (e.g. `__EMAIL__`, `__PHONE_TEL__`, `__LINKEDIN__`, `__SITE_URL__`) — never hardcode the literal value in a source file. `src/build/placeholders.js` is the single module computing every token (including derived/obfuscated variants like `__EMAIL_OBFUSCATED__` and `__TWITTER_HANDLE__`); `minify.js`, `inject-seo.js`, `generate-llms.js`, and `generate-robots.js` all call its `buildReplacements`/`applyReplacements`. To change contact info, edit `package.json` only — never grep-and-replace across HTML/JSON/txt files. `llms.txt` and `robots.txt` at the repo root are **templates** (committed with literal `__PLACEHOLDER__` text, same as `about.html`'s `__BUILD_DATE__`); their resolved output goes to `dist/`, and the CI workflow (`mv dist/llms.txt llms.txt`, `mv dist/robots.txt robots.txt`) promotes the resolved version to root only in the ephemeral deploy runner, never committed.

**Known trade-off**: `npm test` and any local preview serve the raw root files directly (`npx serve .`, see `playwright.config.js`), so `href`/`src` attributes built from placeholders (e.g. `href=__EMPLOYER_URL__`) are literal, unresolved, non-navigable strings until `npm run deploy` has run. Always run `npm run deploy` before manually clicking through links in a local preview. This does not affect the deployed site (CI always resolves before publishing).
